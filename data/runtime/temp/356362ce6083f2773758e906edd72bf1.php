<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:52:"./application/admin/template/shops/order_details.htm";i:1578479372;s:72:"/www/wwwroot/crms.cdqifa.cn/application/admin/template/public/layout.htm";i:1571728726;s:69:"/www/wwwroot/crms.cdqifa.cn/application/admin/template/member/bar.htm";i:1578276580;s:72:"/www/wwwroot/crms.cdqifa.cn/application/admin/template/public/footer.htm";i:1571728726;}*/ ?>
<!doctype html>
<html>
<head>
        ...
        <div class="mDiv">
            ...
            <div class="sDiv">
                <div class="sDiv2 addartbtn fl" style="margin-right: 6px;">
                    <input type="button" class="btn current" value="返回列表" onclick="window.location.href='<?php echo url("Shops/index"); ?>';">
                </div>
            </div>
        </div>
        <form class="form-horizontal" id="postForm" action="<?php echo url('Shops/order_mark_status'); ?>" method="post">
      ...
      <!--                       <td class="gray_bg">订单状态：</td>
                            <td>
                                <?php echo (isset($admin_order_status_arr[$OrderData['order_status']]) && ($admin_order_status_arr[$OrderData['order_status']] !== '')?$admin_order_status_arr[$OrderData['order_status']]:''); if($OrderData['order_status'] == '2'): ?>
                                    &nbsp;<a href="javascript:void(0);" data-url="<?php echo url('Shops/order_send',array('order_id'=>$OrderData['order_id'])); ?>" onclick="sendOrderDetails(this);" class="ncap-btn ncap-btn-green" >
                                        发货详情
                                    </a>
                                    <script type="text/javascript">
                                        // 发货、标记未付款
                    ...
                    <script type="text/javascript">
                        $(function(){
                            $('.remove_dishes').click(function(){
                                ...
                                layer.confirm('<font color="#ff0000">确定要移除该菜品吗？</font><br>(移除后订单总价会减去该菜品价格)', {
                                    ...
                                    layer_loading('正在处理');
                                    // 确定
                                    $.post('<?php echo url("shops/remove_dishes"); ?>',{details_id:details_id},function(data){
                                        layer.closeAll();
                                         if(data.code == 1){
                        ...
                        $('#print_order').click(function(){
                            ...
                            layer.open({
                              ...
                              area: ['400px', '600px'],
                              content: "<?php echo url('Shops/print_order',array('order_id'=>$OrderData['order_id'])); ?>"
                            });
                            })
...
<script type="text/javascript">
    ...
    function OrderMark(status_name){
        if('qfh' == status_name){
            ...
            var url = "<?php echo url('Shops/order_send'); ?>";
            if (url.indexOf('?') > -1) {
                url += '&';
            } else {
                url += '?';
    ...
    function UpNote(order_id){
        ...
        $.ajax({
            url: "<?php echo url('Shops/update_note', ['_ajax'=>1]); ?>",
